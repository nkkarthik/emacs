...
static int settermprop(VTermProp prop, VTermValue *val, void *user)
{
  ...
  case VTERM_PROP_ALTSCREEN:
    ...
    /* When leaving altscreen, push all rows to scrollback before swapping buffers.
     * screen->buffer still points at the altscreen here, so sb_pushline_from_row
     * reads the right content via vterm_screen_get_cell. */
    if(!val->boolean && screen->buffers[BUFIDX_ALTSCREEN] &&
       screen->buffer == screen->buffers[BUFIDX_ALTSCREEN] &&
       screen->callbacks && screen->callbacks->sb_pushline) {
