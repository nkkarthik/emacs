...
PYTHON ?= $(if $(filter Linux,$(OS)),$(HOME)/venvs/vllm/bin/python,python3)
...
python-deps:
	...
	$(PYTHON) -m pip install -r a/llm/requirements.txt
...
# Copy personal elisp packages from a/ into the load path seen by the
# locally-built Emacs binary. On macOS this is the installed
# Emacs.app's site-lisp/ tree; on Linux it is a sibling site-lisp/
# directory next to lisp/, which src/emacs picks up automatically.
# Excludes git-crypt/git metadata and CLAUDE.md so only package code
# lands in the target. a/daemon/ holds launchd/systemd unit files (not
# elisp), so it is excluded too.
NS_APPRESDIR := $(CURDIR)/nextstep/Emacs.app/Contents/Resources
LOCAL_SITELISP := $(CURDIR)/site-lisp
ifeq ($(OS),Darwin)
SITE_LISP_DEST := $(NS_APPRESDIR)/site-lisp
else
SITE_LISP_DEST := $(LOCAL_SITELISP)
endif

.PHONY: install-a site-lisp-sync
install-a: site-lisp-sync
	@emacsclient --no-wait -e \
	  '(dolist (f (file-expand-wildcards "$(CURDIR)/a/util/k8s/k8s*.el")) (load f nil t t))' \
	  2>/dev/null && echo "✅ reloaded k8s in running Emacs" || true

site-lisp-sync: vterm-module
ifeq ($(OS),Darwin)
	@test -d $(SITE_LISP_DEST) || { echo "❌ $(SITE_LISP_DEST) not found — run 'make install' first"; exit 1; }
else
	@mkdir -p $(SITE_LISP_DEST)
endif
	@test -f $(CURDIR)/x/org-roam/org-roam.el || { echo "❌ $(CURDIR)/x/org-roam missing — run 'git submodule update --init x/org-roam'"; exit 1; }
	...
	@rm -rf $(SITE_LISP_DEST)/emacs-libvterm/build
	rsync -a \
		--exclude='.git*' \
		--exclude='CLAUDE.md' \
		--exclude='*.org' \
		...
		--exclude='daemon/' \
		--exclude='build/' \
		$(CURDIR)/a/ $(SITE_LISP_DEST)/
	@echo "✅ rsync'd a/ -> $(SITE_LISP_DEST)/ (excluded data/docs/tests/.elc)"
	rsync -a \
		--exclude='.git*' --exclude='*.org' --exclude='*~' \
		--exclude='doc/' --exclude='tests/' \
		$(CURDIR)/x/org-roam/ $(SITE_LISP_DEST)/org-roam/
	@echo "✅ rsync'd x/org-roam -> $(SITE_LISP_DEST)/org-roam/"
	@printf '%s\n' \
		';;; Auto-generated by e.mk site-lisp-sync.' \
		'(normal-top-level-add-subdirs-to-load-path)' \
		> $(SITE_LISP_DEST)/subdirs.el
	@echo "✅ generated $(SITE_LISP_DEST)/subdirs.el"
ifeq ($(OS),Darwin)
	...
	@ln -snf $(SITE_LISP_DEST) $(LOCAL_SITELISP)
	@echo "✅ symlinked $(LOCAL_SITELISP) -> $(SITE_LISP_DEST)"
endif
...
# require ABI 15.
Linux: ldeps python-deps tree-sitter-src site-lisp-sync
	./autogen.sh
