...
- `install-a` also drops a `site-lisp/` symlink at the repo root pointing at `nextstep/Emacs.app/Contents/Resources/site-lisp/`. Without this symlink the uninstalled `src/emacs` (and any daemon started via `brew-bin`/`local-bin`, which symlink `/{opt,usr/local}/bin/emacs` -> `src/emacs`) sees only a bogus relative `Contents/Resources/site-lisp` on `load-path` and never loads the bundled `a/` packages. `.gitignore` already excludes `/site-lisp/`.
